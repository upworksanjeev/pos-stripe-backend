...
app.post("/api/register-reader", async (req, res) => {
    try {
        ...
        if (error.code === 'resource_missing') {
            res.status(404).json({ error: "Location not found. Please check the location ID." });
        } else if (error.code === 'invalid_request_error') {
            res.status(400).json({ error: "Invalid registration code or location ID" });
        } else {
            res.status(500).json({ 
...
app.post("/api/create-payment-intent", async (req, res) => {
    try {
        ...
        if (!amount || amount <= 0) {
            return res.status(400).json({ error: "Valid amount is required (must be greater than 0)" });
        }
...
app.post("/api/process-payment", async (req, res) => {
    try {
        ...
        if (!readerId) {
            return res.status(400).json({ error: "Reader is required please ensure reader is connected" });
        }
        ...
        if (error.code === 'resource_missing') {
            ...
            res.status(409).json({ error: "Reader is currently busy with another operation" });
        } else if (error.code === 'terminal_reader_offline') {
            res.status(503).json({ error: "Reader is offline. Please check the connection." });
        } else {
            res.status(500).json({ 
...
app.post("/api/invoices/:id/pay", async (req, res) => {
    try {
        ...
        if (error.code === 'resource_missing') {
            ...
            res.status(400).json({ error: "Invalid invoice or customer information" });
        } else {
            res.status(500).json({ 
